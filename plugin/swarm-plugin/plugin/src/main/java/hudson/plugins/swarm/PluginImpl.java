...
public class PluginImpl extends Plugin {
     ...
     * @throws IOException
     */
    public void doAddSystemLoadAverage(StaplerRequest req, StaplerResponse rsp, @QueryParameter String name,
                                       @QueryParameter String secret, @QueryParameter String systemLoadAverage) throws IOException {
        if (!getSwarmSecret().equals(secret)) {
            rsp.setStatus(SC_FORBIDDEN);
            return;
        }
        ...
        nn.setSystemLoadAverage(systemLoadAverage);
        normalResponse(req, rsp, nn.getSystemLoadAverage());
    }
