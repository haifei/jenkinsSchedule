...
public class ThinBackupPluginImpl extends Plugin {
    ...
    /**
     *  2020-05-07 add by wanghf
     */
    private boolean backupDir = false;
    ...
    public void setBackupDir(final boolean backupDir) {
    ...
    public void setBackupDirPath(final String backupDirPath) {
    ...
    public FormValidation doCheckBackupDirPath(final StaplerRequest res, final StaplerResponse rsp,
                                               @QueryParameter("value") final String backupDirPath) {

        if ((backupDirPath == null) || (backupDirPath.isEmpty())) {
            return FormValidation.ok();
        }

        if (backupDirPath.trim().isEmpty()) {
            return FormValidation.warning("Regex is valid, but consists entirely of whitespaces - is this intentional?");
        }

        if(!new File(backupDirPath).exists()){
            return FormValidation.warning("文件路径不存在，请重新确认！！");
        }

        if (!backupDirPath.trim().equals(backupDirPath)) {
            return FormValidation
                    .warning("Regex is valid, but contains leading and/or trailing whitespaces - is this intentional?");
        }
