 ...
 */
public class NoMonthTriggerDownstream extends AbstractTriggerDownstream {

    private static final Logger logger = LoggerFactory.getLogger(NoMonthTriggerDownstream.class);

    private TriggerConditionInfo conditionInfo;

    public NoMonthTriggerDownstream(Job job,
    ...
    public boolean check() {
        ...
        if (date.contains(Constants.DATE_RELY_ON_ONESELF)) {
            ...
            return checkTokenMonthCondition(Utils.stringConvertInteger(date), true);
        }

        if (date.contains(Constants.DATE_OF_CALCULATION_CONTEXT)) {
            date = dateAppendToContext(date);
            return checkTokenMonthCondition(Utils.stringConvertInteger(date), false);
        }

        return checkMonthCondition(Utils.stringConvertInteger(date));
    }
}
